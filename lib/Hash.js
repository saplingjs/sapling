...
try {
	...
	throw new SaplingError('Cannot load required dependency: `crypto`');
}
...
module.exports = class Hash {
	...
	async hash(password, salt) {
		...
		if (arguments.length === 2) {
			...
			if (!password) {
				return new SaplingError('Password missing');
			}

			if (!salt) {
				return new SaplingError('Salt missing');
			}
		...
		return new Promise((resolve, reject) => {
			crypto.randomBytes(this.length, (err, salt) => {
				/* Return any error */
				if (err) {
					return reject(new SaplingError(err));
				}
