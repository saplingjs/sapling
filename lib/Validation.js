...
module.exports = class Validation {
	...
	validateValues(value, key, rule) {
		...
		if (rule.values && (rule.required || !rule.default || value !== '') && !rule.values.includes(value)) {
			this.errors.push({
				status: '422',
				code: '1003',
				title: 'Invalid Value',
	...
	validateFileType(file, key, rule) {
		if (rule.filetype) {
			...
			if (!(file.mimetype in acceptedTypes || file.extension in acceptedTypes || file.group in acceptedTypes)) {
				this.errors.push({
					status: '422',
					code: '2001',
