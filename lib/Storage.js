...
module.exports = class Storage {
	...
	async createDatabase(opts) {
		...
		this.db = new (require(`@sapling/db-driver-${String(this.config.db.type).toLowerCase()}`))(opts);
	...
	async get(req, res) {
		...
		await this.db.read(req.collection, conditions, options, references).then((err, arr) => {
			if (err) {
	...
	async post(req, res) {
		...
		/* If the permission is "owner", check that we are the owner, or an admin */
			...
			await this.db.modify(req.collection, conditions, data);
			...
			/* Send to the database */
			await this.db.write(req.collection, conditions, data);
		}
	}
