...
module.exports = class Storage {
	...
	async createDatabase(opts) {
		...
		if(driver == 'memory') {
			this.db = new (require('../drivers/db/Memory'))(opts);
		} else {
			this.db = new (require(`@sapling/db-driver-${driver}`))(opts);
		}
	...
	getConstraints(req) {
		...
		if (req.type == "filter") {
			...
			if(req.fields.length == req.values.length) {
				for (let index = 0; index < req.fields.length; ++index) {
					let values = req.values[index].split(',');
					if(values.length == 1)
						values = values[0];

					conditions[req.fields[index]] = values;
				}
			}
		}
	...
	getCreatorConstraint(req, role) {
		...
		if (req.permission && req.permission.role.includes("owner") && role !== "admin") {
	...
	async get(req, res) {
		...
		/* Get the collection definition */
		const rules = this.schema[req.collection] || {};
		let options = {};
		let conditions = {};
		...
		/* TODO: convert to standard User method */
		...
		try {
			let arr = await this.db.read(req.collection, conditions, options, references);

			/* Get the list of fields we should not be able to see */
			const omit = this.disallowedFields(role, req.collection);
	
			/* Get the list of fields that only owners can see */
			const ownerFields = this.ownerFields(req.collection);
	...
	async post(req, res) {
		...
		/* Check if we're logged in */
		/* TODO: convert to standard User method */
		if (req.permission && !req.permission.role.includes("anyone") && !req.permission.role.includes("stranger") && !req.session.user) {
			new Response(this.app, req, res, new SaplingError({
				"status": "401",
				"code": "4002",
				"title": "Unauthorized",
		...
		if (errors) {
			new Response(this.app, req, res, new SaplingError(errors));
			return false;
		}
		...
		if (req.files) {
			for(let fileField of Object.keys(req.files)) {
				...
				if((fileField in Object.keys(rules) && rules[fileField].type === 'file') || !this.config.strict) {
					...
					let fileGroup = 'other';
					for(let type of Object.keys(upload_types)) {
					...
					if(rule) {
						...
						if(rule.filetype) {
							...
							if(!(file.mimetype in acceptedTypes || fileExtension in acceptedTypes || fileGroup in acceptedTypes)) {
								new Response(this.app, req, res, new SaplingError({
									...
									"detail": `\`${fileField}\` must be one of ${acceptedTypes.join(', ')}.`,
									"meta": {
										"type": "file",
										"error": "type"
									}
						...
						if(rule.maxsize) {
							...
							if(typeof sizeLimit === 'string') {
								...
								let sizeParts = sizeLimit.match(/^([\.0-9]+)([B|K|M|G)])$/gi);
							...
							if(file.size > sizeLimit) {
								new Response(this.app, req, res, new SaplingError({
									...
									"detail": `\`${fileField}\` must not be larger than ${sizeLimit} bytes.`,
									"meta": {
										"type": "file",
										"error": "maxsize"
									}
					...
					if(fileGroup == 'image') {
						let dimensions = await imageSize(file.tempFilePath);
						fileObject.width = dimensions.width;
						fileObject.height = dimensions.height;

						if(rule) {
							if(fileObject.width < rule.minwidth) {
								new Response(this.app, req, res, new SaplingError({
									"status": "422",
									"code": "2003",
									"title": "Image Too Small",
									"detail": `Width of \`${fileField}\` must be at least ${rule.minwidth} pixels.`,
									"meta": {
										"type": "file",
										"error": "minwidth"
									}
							...
							if(fileObject.width > rule.maxwidth) {
								new Response(this.app, req, res, new SaplingError({
									"status": "422",
									"code": "2004",
									"title": "Image Too Large",
									"detail": `Width of \`${fileField}\` must not be larger than ${rule.maxwidth} pixels.`,
									"meta": {
										"type": "file",
										"error": "maxwidth"
									}
							...
							if(fileObject.height < rule.minheight) {
								new Response(this.app, req, res, new SaplingError({
									"status": "422",
									"code": "2005",
									"title": "Image Too Small",
									"detail": `Height of \`${fileField}\` must be at least ${rule.minheight} pixels.`,
									"meta": {
										"type": "file",
										"error": "minheight"
									}
							...
							if(fileObject.height > rule.maxheight) {
								new Response(this.app, req, res, new SaplingError({
									"status": "422",
									"code": "2006",
									"title": "Image Too Large",
									"detail": `Height of \`${fileField}\` must not be larger than ${rule.maxheight} pixels.`,
									"meta": {
										"type": "file",
										"error": "maxheight"
									}
		...
		if (req.type == "filter") {
			...
			return await this.db.modify(req.collection, conditions, data);
			...
			/* Send to the database */
			return await this.db.write(req.collection, data);
		}
	}
	...
	async delete(req, res) {
		...
		return await this.db.remove(req.collection, conditions);
	}
};
