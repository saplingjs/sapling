...
module.exports = class Storage {
	...
	async createDatabase(opts) {
		...
		const driver = String(this.config.db.type).toLowerCase();
	...
	async get(req, res) {
		...
		return await this.db.read(req.collection, conditions, options, references).then(arr => {
	...
	async post(req, res) {
		...
		/* If the permission is "owner", check that we are the owner, or an admin */
			...
			/* Send to the database */
			return await this.db.write(req.collection, conditions, data);
		}
	}
