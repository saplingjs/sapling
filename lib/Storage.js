...
export default class Storage {
	...
	async createDatabase(options) {
		...
		if (driver === 'memory') {
			this.db = new (await import('../drivers/db/Memory.js')).default(options);
		} else {
			try {
				this.db = new (await import(`@sapling/db-driver-${driver}`)).default(options);
			} catch {
				try {
					this.db = new (await import(driver)).default(options);
				} catch {
	...
	disallowedFields(role, collection) {
		...
		for (const key in rules) {
			if ({}.hasOwnProperty.call(rules, key)) {
				...
				if (this.app.user.isRoleAllowed(role, access.r) === false) {
					omit.push(key);
				}
			}
		}
