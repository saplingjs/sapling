...
module.exports = class Storage {
	...
	async get(request, res) {
		...
		try {
			...
			new Response(this.app, request, res, new SaplingError(error));
		}
	}
	...
	async post(request, res) {
		...
		if (errors) {
			new Response(this.app, request, res, new SaplingError(errors));
			return false;
		}
			...
			for (const fileField of Object.keys(request.files)) {
				...
				if ((fileField in Object.keys(rules) && rules[fileField].type === 'file') || !this.config.strict) {
					...
					if (validator.errors.length > 0) {
						new Response(this.app, request, res, new SaplingError(validator.errors));
						return false;
					}
