...
export default class Templating {
	...
	constructor(App, viewsPath) {
		...
		(async () => {
			if (driver === 'html') {
				this.renderer = new (await import('../drivers/render/Html.js')).default(App, this.viewsPath);
			} else {
				try {
					this.renderer = new (await import(`@sapling/render-driver-${driver}`)).default(App, this.viewsPath);
				} catch {
					try {
						this.renderer = new (await import(driver)).default(App, this.viewsPath);
					} catch {
