...
module.exports = class User {
	...
	constructor(App) {
		...
		this.app.server.get("/api/user/logged", this.getLogged.bind(this));
		this.app.server.post("/api/user/login", this.login.bind(this));
		this.app.server.post("/api/user/update", this.update.bind(this));
		this.app.server.post("/api/user/forgot", this.forgot.bind(this));
		this.app.server.get("/api/user/logout", this.logout.bind(this));
		this.app.server.get("/api/user/recover", this.recover.bind(this));
		this.app.server.post("/api/user/register", this.register.bind(this));
	}
	...
	async login(req, res) {
		...
		const permission = this.getRolesForRoute("get", url);
	...
	*/
	register(req, res) {
		...
		if (err.length) { 
			new Response(this.app, req, res, err);
			return false;
		}

		Hash.hash(req.body.password.toString(), (err, hash) => {
	...
	async forgot(req, res) {
		...
		key += this.app.utils.randString(); // a touch of randomness
	...
	async recover(req, res) {
		...
		const newpass = this.app.utils.randString();

		const hash = await Hash.hash(newpass);
